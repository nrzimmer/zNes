...
#define SET_mos_OVERFLOW(x) (x ? (status |= mos_OVERFLOW) : (status &= (~mos_OVERFLOW)) )
...
#define IF_mos_OVERFLOW() ((status & mos_OVERFLOW) ? true : false)
...
public:
    ...
    std::string InstrName(uint8_t inst);
